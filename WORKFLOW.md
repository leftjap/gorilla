...
| 대상 파일 | 작업지시서 표기 |
|---|---|
| index.html | `C:\dev\workout\index.html` |
| style.css | `C:\dev\workout\style.css` |
| js/storage.js | `C:\dev\workout\js\storage.js` |
| js/data.js | `C:\dev\workout\js\data.js` |
| js/ui.js | `C:\dev\workout\js\ui.js` |
| js/workout.js | `C:\dev\workout\js\workout.js` |
| js/stats.js | `C:\dev\workout\js\stats.js` |
| js/app.js | `C:\dev\workout\js\app.js` |
| WORKFLOW.md | `C:\dev\workout\WORKFLOW.md` |

**규칙:**
- `파일: style.css` ❌ → `파일: C:\dev\workout\style.css` ✅
...
**PowerShell 치환 예시:**
```powershell
$file = 'C:\dev\workout\WORKFLOW.md'
...
e. 배포 URL(https://leftjap.github.io/gorilla/)로 접속 상태를 확인한다 (200 vs 404).
...
| 항목 | 값 |
|---|---|
| 배포 URL | `https://leftjap.github.io/gorilla/` |
| GitHub raw base | `https://raw.githubusercontent.com/leftjap/gorilla/main/` |
...
사용할 것:
  `raw.githubusercontent.com/leftjap/gorilla/main/{파일경로}`
...
### WORKFLOW.md 크롤링 시 주의
- 문서가 클 경우 크롤러가 앞부분만 반환할 수 있음
- 사용자가 직접 업로드한 WORKFLOW.md를 우선 참조한다
...
- 압축 후에는 WORKFLOW.md의 상세 맵(7번)에 있는 함수명/변수명을
...
- 작업지시서 실행 후 결과 보고 → 재수정이 필요한 경우
  (직전 작업의 맥락은 WORKFLOW.md와 git log에 이미 기록되어 있으므로
  새 세션에서 WORKFLOW.md만 업로드하면 이어서 작업 가능)
